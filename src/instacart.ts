 ...
 */
export function dominantCategoryFilter(products: Product[], query: string): Product[] {
  ...
  for (const p of relevant) {
    if (p.category) votes.set(p.category, (votes.get(p.category) || 0) + 1);
  }
  if (votes.size === 0) return relevant; // no taxonomy signal → keyword only
  let top = "";
  let topN = 0;
  for (const [cat, n] of votes) {
    if (n > topN) { top = cat; topN = n; }
  }
  ...
  if (topN < 2 && votes.size > 1) return relevant;
