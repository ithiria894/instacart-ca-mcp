...
const server = new Server(
  { name: "instacart-ca-mcp", version: "0.2.0" },
  { capabilities: { tools: {} } }
);
...
function toShopId(store: string): string {
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "List every retailer available for delivery in the zone (live), with shopId, slug, name and type. Use this to discover stores beyond the built-in set.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    name: "instacart_categories",
    description:
      "List a store's category collection slugs (e.g. meat-and-seafood, produce, frozen). Pass a store name (" +
      STORE_NAMES +
      ") or a retailer slug.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Browse a category at a store — CLEAN, category-accurate results (no cross-category noise). Args: store, category (slug or common name like 'meat-and-seafood'), maxResults.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Fuzzy search a single store. Faster but may include off-category items; for clean results use instacart_browse.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Compare a product's price across stores, cheapest first. Default uses fuzzy search; pass `category` to browse that category collection at each store for clean, noise-free comparison. Optionally limit to a subset of stores.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        category: {
          type: "string",
          description:
            "Optional category (e.g. 'meat-and-seafood', 'produce') — enables clean category-mode comparison",
        },
        stores: {
          type: "array",
          items: { type: "string" },
  ...
  },
  {
    ...
    description:
      "On-sale items at a store (with struck-through original prices). Optionally scope to a query like 'milk' or 'beef'. Args: store, query (optional), maxResults.",
    inputSchema: {
      type: "object",
      properties: {
        store: { type: "string", description: "Store name: " + STORE_NAMES },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "instacart_browse") {
      ...
      if (!slug) slug = store; // allow a raw retailer slug
      if (!shopId) shopId = toShopId(store);
      const slugs = await getCategories(slug);
    ...
    if (name === "instacart_compare") {
      ...
      const stores = Array.isArray(args.stores) ? (args.stores as string[]) : undefined;
