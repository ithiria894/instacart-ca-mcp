...
async function findInstacartTarget() {
    const targets = await CDP.List({ host: DEBUG_HOST, port: DEBUG_PORT });
    log(`Found ${targets.length} CDP targets`);
    const page = targets.find((t) => t.type === "page" && t.url.includes("instacart.ca"));
    if (page) {
 ...
 */
export async function evalInInstacartPage(fnBody) {
    let client;
    try {
...
export async function checkConnection() {
    const res = await evalInInstacartPage(`
