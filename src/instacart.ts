...
async function searchOnSale(shopId: string, query: string, first: number) {
  const variablesExpr = `{
  ...
  `;
  const res = await evalInInstacartPage<{ ok: boolean; products: Product[]; error?: string }>(fnBody);
  if (!res.ok) return { ok: false, products: [] as Product[], error: res.error };
  return res.value!;
}
...
export async function getDeals(
  ...
  for (const term of terms) {
    ...
    for (const p of r.products) {
      if (query.trim() && !isRelevant(query, p.name)) continue;
      if (seen.has(p.name)) continue;
      seen.add(p.name);
      deals.push(p);
    }
    if (deals.length >= first) break;
  }
